...
export function MangaWatchlistButton({
  malId,
  title,
  volumes,
  chapters,
  ...
  const [entry, setEntry] = useState<MangaWatchlistEntry>(initialEntry ?? defaultEntry);
  ...
  useEffect(() => {
    ...
    async function preloadSavedState() {
      try {
        const response = await fetch(`/api/v1/manga-watchlist?malId=${malId}`, {
  ...
  async function loadEntry() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to load manga watchlist entry");
      }
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/v1/manga-watchlist", {
        method: "POST",
        headers: {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || "Failed to save manga watchlist entry");
      }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to delete manga watchlist entry");
      }
  ...
  return (
    <>
      <button
        ...
        className={
          triggerClassName ??
          "inline-flex items-center gap-2 rounded-full border border-zinc-900 bg-white px-3 py-2 text-xs font-black uppercase tracking-[0.2em] text-zinc-900 transition hover:bg-indigo-600 hover:text-white dark:border-white dark:bg-zinc-950 dark:text-zinc-100"
        }
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <button
                    ...
                    aria-label="Close manga watchlist modal"
                  >
                    <X size={18} />
                  </button>
                </div>
                  ...
                  <form className="mt-6 space-y-5" onSubmit={handleSave}>
                    ...
                    <div className="grid gap-5 md:grid-cols-2">
                      <label className="flex flex-col gap-2 text-sm font-semibold text-zinc-200">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setEntry((current) => ({
                              ...current,
                              status: event.target.value as MangaWatchlistEntry["status"],
                            }))
                          }
                      ...
                      <label className="flex flex-col gap-2 text-sm font-semibold text-zinc-200">
                        ...
                        <input
                          type="number"
                          min={0}
                          max={volumes ?? undefined}
                          value={entry.volumesRead}
                          onChange={(event) =>
                            setEntry((current) => ({
                              ...current,
                              volumesRead: Number.parseInt(event.target.value || "0", 10),
                            }))
                          }
                          className="rounded-2xl border border-zinc-700 bg-zinc-900 px-4 py-3 text-sm text-white outline-none transition focus:border-indigo-500"
                        />
                        <span className="text-xs font-medium text-zinc-500">
                          {entry.volumesRead} / {volumes ?? "?"} volumes
                        </span>
                      </label>
                      ...
                      <label className="flex flex-col gap-2 text-sm font-semibold text-zinc-200">
                        ...
                        <input
                          type="number"
                          min={0}
                          max={chapters ?? undefined}
                          value={entry.chaptersRead}
                          onChange={(event) =>
                            setEntry((current) => ({
                              ...current,
                              chaptersRead: Number.parseInt(event.target.value || "0", 10),
                            }))
                          }
                          className="rounded-2xl border border-zinc-700 bg-zinc-900 px-4 py-3 text-sm text-white outline-none transition focus:border-indigo-500"
                        />
                        <span className="text-xs font-medium text-zinc-500">
                          {entry.chaptersRead} / {chapters ?? "?"} chapters
                        </span>
                      </label>
                    ...
                    <div className="flex flex-col-reverse gap-3 border-t border-zinc-800 pt-4 sm:flex-row sm:items-center sm:justify-between">
                      <button
                        ...
                        disabled={!hasSavedEntry || isDeleting || isSaving}
                        className="inline-flex items-center justify-center gap-2 rounded-2xl border border-red-900 px-4 py-3 text-sm font-semibold text-red-300 transition hover:bg-red-950/40 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                      ...
                      <button
                        type="submit"
                        disabled={isSaving}
                        className="inline-flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-bold text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                        {isSaving ? <LoaderCircle size={16} className="animate-spin" /> : <Bookmark size={16} />}
                        Save entry
                      </button>
                    </div>
                  </form>
