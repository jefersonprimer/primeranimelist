...
export async function generateMetadata({
  ...
  return {
    ...
    description: `Browse the most popular ${genreInfo.label.toLowerCase()} anime on PrimerAnimeList.`,
  };
}

export default async function GenrePopularPage({
  params,
  searchParams,
}: {
  params: Promise<{ genre: string }>;
  ...
  return (
    <GenreListPage
      genre={genre}
      genreInfo={genreInfo}
      pageTitle={`Popular ${genreInfo.label} Anime`}
      subtitle="Sorted by popularity using the same ranking source as the rest of the project."
      anime={result.items}
      currentPage={currentPage}
      totalPages={result.totalPages}
      total={result.total}
      basePath={`/videos/${genre}/popular`}
      activeView="popular"
    />
  );
}
